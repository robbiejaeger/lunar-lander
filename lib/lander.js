function Lander(x, y, ctx) {
  ...
  this.crashedLanderImg = this.getCrashedLanderImage()
...
Lander.prototype.getCrashedLanderImage = function(){
  var image = new Image()
  image.src = 'img/lander-crashed.png'
  return image
}
...
Lander.prototype.draw = function(){
  ...
  if (this.status === "Crashed") {
    this.ctx.drawImage(this.crashedLanderImg, this.x - this.width/2, this.y - this.height/2, this.width, this.height)
  } else {
    this.ctx.drawImage(this.landerImg, this.x - this.width/2, this.y - this.height/2, this.width, this.height)
  }
  if (this.launching) this.ctx.drawImage(this.flameImg, this.x - this.width/2 + 7, this.y + this.height/2 - 7, 15, 30)
  this.ctx.restore()
}
